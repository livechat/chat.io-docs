...
Web API is similar to REST API. A client (server) can send a **request message** that results in getting a **response message**.
...
Real-Time Messaging API (RTM API) is based on a websocket-like connection. A client (server) can send **request message** that results in getting **response message**. It can also get **push messages** anytime.
...
A service which uses chat.io Agent API should ping the server each 15 seconds. If it's not implemented, the connection will be closed after about one minute of inactivity. If [control frame ping](https://tools.ietf.org/html/rfc6455#section-5.5.2) is unavailable (in web browsers), the service should use a protocol message with `ping` action.
...
```js
{
	"id": "0affb00a-82d6-4e07-ae61-56ba5c36f743", // ggenerated server-side
	"custom_id": "12345-bhdsa",
	"order": 1, // generated server-side
	"type": "annotation",
	"author_id": "b7eff798-f8df-4364-8059-649c35c9ed0c",
